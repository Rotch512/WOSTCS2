...
def find_manifest_item(manifest: dict[str, Any], selector: str) -> dict[str, Any] | None:
    normalized = selector.strip()
    for item in manifest.get("demos", []):
        if item.get("file_id") == normalized or item.get("name") == normalized:
            return item
...
def remove_manifest_item(path: Path, selector: str) -> dict[str, Any]:
    manifest = read_json(path, default={"demos": []}) or {"demos": []}
    item = find_manifest_item(manifest, selector)
    if not item:
        raise KeyError(f"Demo not found in manifest: {selector}")
    manifest["demos"] = [row for row in manifest.get("demos", []) if row.get("file_id") != item.get("file_id")]
    manifest["updated_at"] = utc_now_iso()
    write_json(path, manifest)
    return item

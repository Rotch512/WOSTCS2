...
def _failure_counts(items: list[dict[str, Any]], key: str) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def compact_match_row(row: dict[str, Any]) -> dict[str, Any]:
    compact = _without_detail_lists(row)
    compact["detail_path"] = f"match_details/{row['match_id']}.json"
    compact["failure_counts"] = {
        "clutch": _failure_counts(row.get("clutch_failures", []), "state"),
        "advantage": _failure_counts(row.get("advantage_failures", []), "state"),
    }
    compact["round_detail_count"] = len(row.get("round_details", []) or [])
    return compact


def split_summary_payload(summary: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], dict[str, dict[str, Any]]]:
    team = dict(summary.get("team") or {})
    full_matches = list(team.get("matches") or [])
    player_matches = list(summary.get("player_matches") or [])
    players_by_match: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in player_matches:
        players_by_match[str(row.get("match_id", ""))].append(row)
    ...
    for match in full_matches:
        ...
        if not match_id:
            continue
        compact_matches.append(compact_match_row(match))
        details[match_id] = {
            "match": match,
            "players": sorted(players_by_match.get(match_id, []), key=lambda row: row.get("player", "")),
        }

    slim = dict(summary)
    player_summary = {
        "players": slim.pop("players", []),
        "player_quarters": slim.pop("player_quarters", []),
        "player_maps": slim.pop("player_maps", []),
    }
    slim.pop("player_matches", None)
    slim_team = dict(team)
    slim_team["matches"] = sorted(compact_matches, key=lambda row: row["date"])
    slim["team"] = slim_team
    return slim, player_summary, sorted(player_matches, key=lambda row: (row["date"], row["player"])), details
